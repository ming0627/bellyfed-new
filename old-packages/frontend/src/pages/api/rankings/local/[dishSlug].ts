...
/**
 * API endpoint to get local rankings for a dish
 * GET: Get local rankings for a dish
 */
async function handler(
  ...
  // Handle GET request - get local rankings for a dish
  if (req.method === 'GET') {
    try {
      ...
      try {
        ...
        const rankingsQuery = `
          ...
          WHERE dr.dish_id = $1 AND u.country_code = $2
          ORDER BY dr.rank ASC, dr.updated_at DESC
          LIMIT 50
        `;

        const rankingsResult = await client.query(rankingsQuery, [
          dishId,
          countryCode,
        ]);
