...
# Generate the Docker tag using the unified version script
DOCKER_TAG=$(./scripts/version.sh --env=$ENVIRONMENT)
...
./scripts/version.sh --env=$ENVIRONMENT --update-changelog
