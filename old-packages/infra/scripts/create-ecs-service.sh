...
# Script to create an ECS service for the frontend application
# This script is used to recreate the service if it was manually deleted
# Usage: ./create-ecs-service.sh [environment]
...
TARGET_GROUP_NAME="bellyfed-${ENVIRONMENT}-tg"
...
if [ $? -ne 0 ]; then
  ...
  echo "Please run ./create-task-definition.sh first."
  exit 1
fi
echo "Using task definition: ${TASK_DEFINITION}"

# Get the security group for the service
SERVICE_SG=$(aws ssm get-parameter --name "/bellyfed/${ENVIRONMENT}/security-group/service-security-group" --query "Parameter.Value" --output text)
if [ $? -ne 0 ]; then
...
TARGET_GROUP_ARN=$(aws ssm get-parameter --name "/bellyfed/${ENVIRONMENT}/alb/target-group-arn" --query "Parameter.Value" --output text)
if [ $? -ne 0 ]; then
...
aws ecs create-service \
  ...
  --tags key=Environment,value=${ENVIRONMENT} key=Application,value=Bellyfed key=Stack,value=Frontend-Service
