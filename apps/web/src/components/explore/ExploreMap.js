...
const ExploreMap = memo(function ExploreMap({
  ...
  getCountryLink,
  defaultCenter = { lat: 3.1390, lng: 101.6869 }, // Default to Kuala Lumpur
  defaultZoom = 12,
}) {
  ...
  // Handle marker click
  const handleMarkerClick = useCallback((restaurantId) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row gap-3">
        ...
        >
          {isLoadingLocation ? (
            <LucideClientIcon icon={Loader2} className="w-5 h-5 animate-spin mr-2" aria-hidden="true" />
          ) : (
            <LucideClientIcon icon={Navigation} className="w-5 h-5 mr-2" aria-hidden="true" />
          )}
      ...
      <div className="relative h-[500px] bg-gray-100 dark:bg-gray-700">
        {!mapLoaded ? (
          <div className="absolute inset-0 flex items-center justify-center">
            <LucideClientIcon icon={Loader2} className="w-8 h-8 animate-spin text-orange-500" aria-label="Loading map" />
          </div>
        ) : (
          <>
            ...
            <div className="absolute inset-0 bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
              <div className="text-center p-6 max-w-md">
                <div className="w-16 h-16 bg-orange-100 dark:bg-orange-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
                  <LucideClientIcon icon={MapPin} className="w-8 h-8 text-orange-500" aria-hidden="true" />
                </div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                  Interactive Map
                </h3>
                <p className="text-gray-600 dark:text-gray-400 mb-4">
                  In a real application, this would display an interactive map with restaurant markers.
                </p>
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div className="bg-white dark:bg-gray-800 p-3 rounded-md shadow-sm">
                    <div className="text-sm text-gray-500 dark:text-gray-400">Latitude</div>
                    <div className="font-medium text-gray-900 dark:text-white">{mapCenter.lat.toFixed(4)}</div>
                  </div>
                  <div className="bg-white dark:bg-gray-800 p-3 rounded-md shadow-sm">
                    <div className="text-sm text-gray-500 dark:text-gray-400">Longitude</div>
                    <div className="font-medium text-gray-900 dark:text-white">{mapCenter.lng.toFixed(4)}</div>
                  </div>
                </div>
                <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded-md shadow-sm">
                  <div className="text-sm text-gray-500 dark:text-gray-400">Zoom Level</div>
                  <div className="font-medium text-gray-900 dark:text-white">{mapZoom}</div>
                </div>
            ...
            {userLocation && (
              <div className="absolute bottom-4 left-4 bg-white dark:bg-gray-800 p-3 rounded-md shadow-md">
                <div className="flex items-center text-sm">
                  <LucideClientIcon icon={Navigation} className="w-4 h-4 text-orange-500 mr-2" aria-hidden="true" />
