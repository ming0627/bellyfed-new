 ...
 */
const SimilarRestaurantItem = memo(function SimilarRestaurantItem({ restaurant, getCountryLink }) {
  return (
    <Link
...
const SimilarRestaurants = memo(function SimilarRestaurants({
      ...
      return Promise.resolve(
        mockSimilarRestaurants.filter(
          (restaurant) =>
            ...
            restaurant.cuisineTypes?.some((cuisine) => cuisineTypes?.includes(cuisine))
        )
      );
    },
  ...
  if (!cuisineTypes || cuisineTypes.length === 0 || (similarRestaurants && similarRestaurants.length === 0)) {
    return null;
  }
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          ...
          <div className="pt-2 text-center">
            <Link
              href={getCountryLink(`/restaurants?cuisines=${cuisineTypes.join(',')}`)}
