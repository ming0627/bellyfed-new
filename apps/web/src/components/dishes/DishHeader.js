 ...
 */
const DishHeader = memo(function DishHeader({
  dish,
  getCountryLink,
}) {
    ...
    <div className="mb-8">
      ...
      <div className="relative rounded-lg overflow-hidden mb-6">
        ...
        <div className="absolute top-4 right-4 flex space-x-2">
          ...
          >
            <LucideClientIcon icon={Heart} className={`w-5 h-5 ${isFavorite ? 'fill-current' : ''}`} />
          </button>
      ...
      <div>
          ...
          <div className="flex items-center">
            {dish.rating && (
              <div className="flex items-center bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-200 px-3 py-1 rounded-full">
                <LucideClientIcon icon={Star} className="w-4 h-4 mr-1 fill-current text-yellow-500" />
                <span className="font-medium">{dish.rating.toFixed(1)}</span>
                <span className="mx-1 text-gray-500 dark:text-gray-400">•</span>
                <span className="text-sm text-gray-600 dark:text-gray-400">{dish.reviewCount} reviews</span>
              </div>
            )}
          </div>
        </div>
        ...
        {dish.isAvailable === false && (
          <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md">
            <div className="flex items-start">
              <LucideClientIcon icon={Info} className="w-5 h-5 text-gray-500 mr-2 mt-0.5" />
              <div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-1">
                  Currently Unavailable
                </h3>
                <p className="text-gray-700 dark:text-gray-300">
                  This dish is currently not available. Please check back later or contact the restaurant for more information.
                </p>
              </div>
            </div>
          </div>
