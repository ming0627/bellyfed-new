 ...
 * Logger Utility
 *
 * This module provides a comprehensive logging system for the Bellyfed application.
 * It supports different log levels, environment-aware logging, and structured output.
 */

import { getEnvironmentName, isProductionEnvironment, isDevelopmentEnvironment } from './environment.js'
...
export function enableServerLogging(enable) {
  config.enableServerLogging = enable
  log(LogLevel.INFO, 'Logger', `Server logging ${enable ? 'enabled' : 'disabled'}`)
}

/**
 * Enable or disable logging to storage
 * @param {boolean} enable Whether to enable logging to storage
 */
export function enableStorageLogging(enable) {
  config.enableStorageLogging = enable
  log(LogLevel.INFO, 'Logger', `Storage logging ${enable ? 'enabled' : 'disabled'}`)
}
...
function logToConsole(entry) {
  ...
  // Log to console
  if (data !== undefined) {
    console[consoleMethod](`%c${prefix} ${message}`, `color: ${color}`, data)
  } else {
    console[consoleMethod](`%c${prefix} ${message}`, `color: ${color}`)
  }
}
...
export function clearLogs() {
  logStorage.length = 0
  log(LogLevel.INFO, 'Logger', 'Log storage cleared')
}
 ...
 * @returns {number} The number of logs successfully sent
 */
export async function retryFailedLogs() {
  ...
  log(LogLevel.INFO, 'Logger', `Retried ${logsToRetry.length} failed logs, ${successCount} successful`)
  return successCount
}
...
const logger = {
  trace: (message, data) => trace('App', message, data),
  debug: (message, data) => debug('App', message, data),
  info: (message, data) => info('App', message, data),
