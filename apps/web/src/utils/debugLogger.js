...
export function configure(newConfig) {
  ...
  log(
    LogLevel.INFO,
    'DebugLogger',
    'Debug logger configuration updated',
    newConfig,
  );
}
...
export function log(level, category, message, data) {
  ...
  if (
    ...
    (level >= LogLevel.ERROR || isDevelopmentEnvironment())
  ) {
  ...
  if (config.logToServer) {
    ...
    const isServerLogError =
      category === 'DebugLogger' &&
      message.includes('Failed to send log to server');
    if (!isServerLogError) {
...
function logToConsole(entry) {
  ...
  const timeStr = config.includeTimestamp
...
export async function retryFailedLogs() {
  ...
  log(
    LogLevel.INFO,
    'DebugLogger',
    `Retried ${logsToRetry.length} failed logs, ${successCount} successful`,
  );
  return successCount;
}

// Convenience methods for different log levels
export const trace = (category, message, data) =>
...
export const warn = (category, message, data) =>
  log(LogLevel.WARN, category, message, data);
export const error = (category, message, data) =>
  log(LogLevel.ERROR, category, message, data);
export const fatal = (category, message, data) =>
  log(LogLevel.FATAL, category, message, data);
