/**
 * API Route: Get Dish Rankings
 * 
 * This API route retrieves all rankings for a specific dish.
...
/**
 * Handler for dish rankings API endpoint
 * 
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const validSortFields = ['rating', 'createdAt', 'updatedAt', 'userName', 'restaurantName'];
    ...
    // Get dish rankings
    const rankings = await rankingService.getDishRankings(id, {
      page: pageNum,
      limit: limitNum,
      sortBy,
      sortOrder,
    ...
    res.status(500).json({
      ...
      message: 'Failed to fetch dish rankings'
    });
  }
}
